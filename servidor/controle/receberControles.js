...
const { search } = require("../rotas/receberRotas");
...
/* exports.rendercalorias2 = async (req, res) => {
  let data = "";
  //pesquisarTermo
  try {
    let API_ID = "27042fc5";
    ...
    let response = await fetch(
      `https://api.edamam.com/search?app_id=${API_ID}&app_key=${API_KEY}&q=${Search}&from=0&to=10`
    );
...
/* rotas.get("/calories", async function(req,res){
  ...
  try {
    ...
    let response = await fetch(
      `https://api.edamam.com/search?app_id=${API_ID}&app_key=${API_KEY}&q=pizza&from=0&to=10`
    );
    data = await response.json();  
    console.log(data);
  } catch (error) {
